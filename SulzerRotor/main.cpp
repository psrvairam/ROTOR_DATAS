...
public:
    ...
    deque<POINT> read_points_csv(string filename,bool reverse){
  	deque<POINT> points;
...
//  	vector<string> row = csv_read_row(in, '\t');
  	while(in.good())
  	{
       vector<string> row = csv_read_row(in, ',');
       if (row.size() > 0) {
    	   if (reverse) points.push_back(POINT(atof(row[1].c_str()), atof(row[0].c_str()),0));
    	   else points.push_back(POINT(atof(row[0].c_str()), atof(row[1].c_str()),0));
       }
    }
  	in.close();
  	return points;
  ...
  void makeMesh()
  {
    ...
    //blade
    deque<POINT> blade = read_points_csv("rotor_blade.csv", true);
...
//
	...
	deque<POINT> trailtopD = trailtopS.discretize2(ntrailtop,1,trailtop_par1,trailtop_par2, true, false);
	...
	//periodic
	deque<POINT> wall1 = read_points_csv("wall1_R.csv", false);
	...
	if (makemesh){
		...
		mesh2D.UpdateBoundaries();
		mesh2D.findNodeNeighbors();
		...
		for(int i=mesh2D.nfa_i; i<mesh2D.faces.size(); i++)
		{
			if(strcmp (mesh2D.faces[i].name,"noname") == 0)
			{
			  ...
			  if ((fabs(node0.x - inletD[0].x)<1e-8) && (fabs(node1.x - inletD[0].x)<1e-8)) strcpy (mesh2D.faces[i].name,"outletrotor");
			  else if ((fabs(node0.x - outletD[0].x)<1e-6) && (fabs(node1.x - outletD[0].x)<1e-6)) strcpy (mesh2D.faces[i].name,"inletrotor");

			  else if (find(meshpts.begin(), meshpts.end(), node1) != meshpts.end() &&
					find(meshpts.begin(), meshpts.end(), node0) != meshpts.end())
					strcpy (mesh2D.faces[i].name,"bladerotor");
			  else {
				  for (int j=0; j < topD.size()-1;j++){
					  if (  (fabs(node1.x - topD[j].x) < 1e-3) && (fabs(node1.y - topD[j].y) < 1e-3)
						 && (fabs(node0.x - topD[j+1].x) < 1e-3) && (fabs(node0.y - topD[j+1].y) < 1e-3)
						  ...
						  strcpy (mesh2D.faces[i].name,"wall1rotor");
					  }
				  }
				  for (int j=0; j < botD.size()-1;j++){
					  if (  (fabs(node0.x - botD[j].x) < 1e-3) && (fabs(node0.y - botD[j].y) < 1e-3)
						 && (fabs(node1.x - botD[j+1].x) < 1e-3) && (fabs(node1.y - botD[j+1].y) < 1e-3)
						  ...
						  strcpy (mesh2D.faces[i].name,"wall2rotor");
					  }
				  }
			  }
			}
		...
		BC_names.push_back("inletrotor");
		BC_names.push_back("outletrotor");
		BC_names.push_back("wall1rotor");
		BC_names.push_back("wall2rotor");
		BC_names.push_back("bladerotor");
		mesh2D.writeSU2(su2mesh.c_str(),2, BC_names);
		BC_names.push_back("fluid");
		mesh2D.ScaleMesh(1000.0);
